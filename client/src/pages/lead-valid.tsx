...
function ArchiveCard({ lead }: { lead: Lead }) {
  const processedAt = lead.validAt || lead.createdAt;
  ...
  return (
    <Card className="overflow-hidden border-white/10 bg-white/[0.04] shadow-[0_22px_40px_-26px_rgba(0,0,0,0.68)]">
      <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3">
        ...
        <Badge className="border-emerald-400/20 bg-emerald-500/12 text-emerald-200">
        ...
        <div className="grid gap-4 md:grid-cols-[1fr_0.92fr]">
          <div className="space-y-3">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-3">
              <p className="text-[11px] uppercase tracking-[0.24em] text-slate-500">信息摘要</p>
              <p className="mt-2 text-sm leading-relaxed text-slate-300">
                {lead.detail || "已完成好友确认并进入有效池。"}
              </p>
            </div>
          </div>
...
export default function LeadValidPage() {
  const query = useQuery({
    queryKey: ["lead", "valid"],
    queryFn: getLeadValid,
    staleTime: 30_000,
  });
  ...
  return (
    <div className="min-h-[100dvh] bg-[radial-gradient(circle_at_top,rgba(52,211,153,0.12),transparent_35%),linear-gradient(180deg,#0b1220_0%,#060b14_100%)] text-white">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 px-4 pb-8 pt-4 sm:px-5 lg:px-6">
        <LeadQueueHeader
          title="已有效"
          description="已加好友的线索会进入有效池，方便后续回访与转化追踪。"
          stats={[
            { label: "已有效", value: summary.valid ?? items.length, hint: "Valid" },
            { label: "总数", value: summary.total ?? items.length, hint: "All" },
            { label: "最近处理", value: items[0] ? formatLeadTimestamp(items[0].validAt || items[0].createdAt) : "—", hint: "Recent" },
        ...
        {loading ? (
          ...
          <Card className="border-white/10 bg-white/[0.04] p-6 text-center">
            <p className="text-lg font-semibold text-white">已有效列表加载失败</p>
          ...
          <Card className="border-white/10 bg-white/[0.04] p-8 text-center">
            <p className="text-lg font-semibold text-white">暂无有效线索</p>
            <p className="mt-2 text-sm leading-relaxed text-slate-400">
              当前还没有确认添加好友的线索，处理完成后会在这里出现。
            </p>
          </Card>
        ) : (
